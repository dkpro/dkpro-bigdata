...
import de.tudarmstadt.ukp.dkpro.bigdata.collocations.CollocMapper.Count;

/**
 * Reducer for pass 2 of the collocation discovery job. Collects ngram and sub-ngram frequencies and
 * performs the Log-likelihood ratio calculation.
 */
public class AssocReducer
    extends Reducer<Gram, Gram, Text, DoubleWritable>
{
     ...
     * total
     * 
     * Each ngram will have 2 subgrams, a head and a tail, referred to as A and B respectively
     * below.
     * 
     * A+ B: number of times a+b appear together: ngramFreq A+!B: number of times A appears without
     * B: hSubgramFreq - ngramFreq !A+ B: number of times B appears without A: tSubgramFreq -
     * ngramFreq !A+!B: number of times neither A or B appears (in that order): N - (subgramFreqA +
        ...
        throws IOException, InterruptedException
    {
        ...
        log.info("NGram Total: {}, Min DICE value: {}, Emit Unigrams: {}", new Object[] {
                ngramTotal, minValue, emitUnigrams });
    ...
    /**
     * provide interface so the input to the DICE calculation can be captured for validation in unit
     * testing
     */
    ...
    public static final class DiceCallback
        implements AssocCallback
    {
    ...
    public static final class PMICallback
        implements AssocCallback
    {
        ...
        public double assoc(long k11, long k12, long k21, long k22)
        {
            ...
            // double e22 = (double) ((k22 + k21) * (k22 + k12)) / (double) total;
    ...
    public static final class ChiSquareCallback
        implements AssocCallback
    {
        ...
        public double assoc(long k11, long k12, long k21, long k22)
        {
            ...
            return (n * (k11 * k22 - k21 * k12) / ((k11 + k21) * (k12 + k22) * (k11 + k12) * (k21 + k22)));
        }
    }
