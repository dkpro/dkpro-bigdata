 ...
 *
 */
public class ARCInputFormat extends FileInputFormat<Text, CrawlerRecord> {
	 ...
	 *
	 */
	public static class ARCRecordReader implements RecordReader<Text, CrawlerRecord> {
		private long start;
		private long end;
		 ...
		 */
		public ARCRecordReader(FileSplit split, JobConf jobConf) throws IOException {
		...
		private void fillCrawlerRecord(ArcRecordBase record, CrawlerRecord crawlerRecord) throws IOException {
			// Fill CrawlerRecord
		...
		public boolean next(Text key, CrawlerRecord value) throws IOException {
			...
			while ((arcRecord = arcReader.getNextRecord()) != null) {
				...
				try {
					...
					System.err.println("WARNING: Skipping ARC record (byte offset " + fsin.getCount() +
							") due to unsupported encoding. The record may contain binary data.");
				} catch (Exception e) {
					// Skip any other records that produce exceptions
					System.err.println("WARNING: Skipping ARC record due to exception that occured while reading record:");
