 ...
 *
 */
public class WARCInputFormat extends FileInputFormat<Text, CrawlerRecord> {
	 ...
	 *
	 */
	public static class WARCRecordReader implements RecordReader<Text, CrawlerRecord> {
		private long start;
		private long end;
		private CountingInputStream fsin;
		WarcReader warcReader = null;
		long lastRecordEnd = -1;
		private JobConf conf = null;
		
		private Set<String> contentTypeWhitelist = new HashSet<String>();
		...
		void configure(Configuration conf) {
			String contentTypeWhitelistStr = conf.get("webcorpus.documentjob.content-type-whitelist", "text/html");
		...
		/**
		 * Creates an ARCRecordReader for the specified <code>split</code> of the input stream that will
		 * start at the first valid ARC record header after <code>split.getStart()</code> and continue
		 * until a record is read that goes past <code>split.getStart() + split.getLength()</code>.
		 */
		public WARCRecordReader(FileSplit split, JobConf jobConf) throws IOException {
			conf = jobConf;
		...
		private void fillCrawlerRecord(WarcRecord record, CrawlerRecord crawlerRecord) throws IOException {
			// Fill CrawlerRecord
			...
			// As soon as we put any of this record content into a string, it is explicitly converted
			// from/to some encoding. So before that, we have to guess the correct encoding, otherwise
		...
		public boolean next(Text key, CrawlerRecord value) throws IOException {
			...
			while ((arcRecord = warcReader.getNextRecord()) != null) {
				...
				// is buffered
				// (meaning that (fsin.getCount() > end) is true before we've read the last record before end)
				...
				try {
					...
					if (conf.getBoolean("webcorpus.common.io.warcinputformat.filter-mimetypes", false)) {
						PayloadWithHeaderAbstract payloadHeader = arcRecord.getPayload().getPayloadHeaderWrapped();
					...
					System.err.println("WARNING: Skipping WARC record (byte offset " + fsin.getCount() +
							") due to unsupported encoding. The record may contain binary data.");
				} catch (Exception e) {
					// Skip any other records that produce exceptions
					System.err.println("WARNING: Skipping WARC record due to exception that occured while reading record:");
		...
		/**
		 * Skips InputStream to next record past <code>start</code> or not at all if start is exactly the start of a record
		 */
        private void skipToNextRecord(long start) throws IOException {
