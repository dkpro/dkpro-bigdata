...
rsync -avc target/org.dkpro.bigdata.collocations-0.2.0-SNAPSHOT.jar  $HEADNODE:$TARGET_DIR/
rsync -avc --delete target/lib/* --exclude "hadoop-*.jar" $HEADNODE:$TARGET_DIR/lib/
RUN="./run.sh org.dkpro.bigdata.collocations.CollocDriver -u --input $1 --output  $2 -wm SENTENCE -ws 3 -minV 3 --metric llr:0  -ow --maxRed 64"
